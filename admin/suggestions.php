    ...
    <div class="row mt-5">
        <div class="col-lg-12 mb-4">
            <div class="border shadow tile-container h-100 d-flex align-content-between w-100 flex-wrap text-white" style="background: #589C1C;">
                <div class="w-100" id="displayy">
                    <strong class="d-block mb-4">
                        Next Cycle Budget <br>
                        (Suggested)
                    </strong>
                    <button class="btn btn-primary" data-low="" data-mid="" data-high="" id="useSuggestionBtn" style="display: none;">Use suggestion</button>
                </div>
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    if (parseFloat(response.data.active_total_budget) != 0) {
                        ...
                        console.log(response.data.high_percentage);
                        console.log(response.data.medium_percentage);
                        console.log(response.data.low_percentage);
                        ...
                        $('#useSuggestionBtn').css('display', 'block');
                        $('#useSuggestionBtn').attr({'data-low': lowBudget.toFixed(2), 'data-mid': mediumBudget.toFixed(2), 'data-high': highBudget.toFixed(2)});
        ...
        $('#useSuggestionBtn').click(function () {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/api/suggestions/UseSuggestionDataAjax.php',
                        data: {
                            parentId: '<?=$id;?>',
                            numberOfCycle: numberOfCycle,
                            is_customized: is_customized,
                        ...
                        dataType: 'json',
                        success: function (response) {
                            console.log(numberOfCycle);
                            if (response.success) {
                                console.log("working here");
                                console.log(response.data);
                                console.log(response.data.high_budget);
                                console.log(response.data.medium_budget);
                                console.log(response.data.low_budget);
                                Swal.fire({
                                    title: "Success!",
