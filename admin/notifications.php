                ...
                while($row = mysqli_fetch_assoc($result)) {
                        ...
                        switch ($row['notification_type']) {
                            ...
                            case "cycle_overspent":
                                ...
                                if ($row['account_type'] == 1) {
                                    $message = $message4 . $message2 . " overspent, having ₱" .$row['savings']. " on your recent cycle add more budget on your next cycle to prevent overspending. <strong>" . /*date("F jS, Y", strtotime($row['start'])) .*/"</strong> <!--to--> <strong>" /*.date("F jS, Y", strtotime($row['end']))*/. "</strong>";
                                } elseif ($row['account_type'] == 2) {
                                    $message = "You have overspent in the " . $row['expenses_name_name'] . " expenses, add more budget on <strong>" . $row['expenses_category_name'] . "</strong> category.";
                                } elseif ($row['account_type'] == 3) {
                                    $message = $message5 . $message2 . " overspent on your recent cycle, having ₱" .$row['savings']. " add more budget on your next cycle to prevent overspending. <strong>" . /*date("F jS, Y", strtotime($row['start'])) .*/"</strong> <!--to--> <strong>" /*.date("F jS, Y", strtotime($row['end']))*/. "</strong>";
                                } else {
                                    $message = "You have overspent in the " . $row['expenses_name_name'] . " expenses, add more budget on <strong>" . $row['expenses_category_name'] . "</strong> category.";
                                }
                                break;
                            case "savings":
                                ...
                                $message = "Great job! you have got ₱" . $row['savings'] . " peso(s) savings on your recent cycle. <strong>" . /*date("F jS, Y", strtotime($row['start'])) .*/ "</strong> <!--to--> <strong>" . /*date("F jS, Y", strtotime($row['end'])) .*/ "</strong>";
                                break;
                            case "no_overspend":
                                $circleColor = '#fff';
                                $border = '2px solid #589C1C';
                                $rowIcon = '<i class="fas fa-check" style="color: #589C1C;"></i>';
                                $message = "Nice, you have followed your category expenses accordingly on your recent cycle. <strong>" . /*date("F jS, Y", strtotime($row['start'])) .*/ "</strong> <!--to--> <strong>" . /*date("F jS, Y", strtotime($row['end'])) .*/ " </strong> <!--cycle.-->";
                                break;
                            default:
                                // code block
                        } ?>
